...
There will be tasks for which espanso was not designed. For those cases, espanso offers the **Shell Extension**, which is similar to the [Script Extension](#script-extension) below, but executes **shell commands**. This offers a lot of flexibility on Unix systems thanks to the `bash` shell (and thanks to PowerShell/WSL support also on Windows).
...
Like the Script extension, the Shell extension can call external shell script files and run inline multiline code.
  ...
  - trigger: ":ip"
    ...
    vars:
      - name: output
        ...
        params:
          cmd: "curl 'https://api.ipify.org'"
          shell: wsl # or shell: bash       
```

Possible values for the `shell` parameter are: `bash`, `cmd`, `fish`, `nu`, `powershell`, `pwsh`, `sh`, `wsl`, `wsl2`, or `zsh`.  
  
- `cmd`, `powershell`, `wsl`, `wsl2` are Windows-specific (they invoke Windows/WSL binaries).  
- `bash`, `fish`, `nu`, `pwsh`, `sh`, `zsh` are not restricted by espanso to any OS — they will be attempted on any platform, but require the corresponding binary to be installed and available in PATH.  

> Note that Windows `cmd` doesn't work with _multiline_ inline code.

### Bash pipes

This extension supports bash **pipes** as your shell does, such as:
...
It's very common for commands to have outputs that also spawn a newline at the end. By default a trim option is enabled to _remove_ any excess spaces/newlines. You can optionally disable the `trim` option to retain them:
...
When triggering a shell or script command, espanso also injects a few useful Environment Variables that you can use:
...
The
**Script Extension**, enables you to call a **script**, written in **any language**,  and use its output in a match.
...
You can do the same thing with any programming language - just change the `args` sequence accordingly.

:::tip script location

The current best-practice when creating Script matches is to create a `scripts` directory in the `espanso` directory
(at the same level as the `match` and `config` directories) and store the scripts there.
...
Because of the execution time, you should limit yourself to fast-running scripts to avoid lag.

:::

[Environment variables](#useful-environment-variables) and [output trimming](#trimming-the-output) work as in the shell extension.
    ...
    vars:
      - name: output
        ...
        params:
          args: [python, -c, *script1]
```
The anchor will also be available for use in this and any other triggers that *follow*.
