...
Espanso automatically localize dates based on your system's country and language, 
...
### UTF-8 characters in Windows
See the section at the bottom of **Shell Extension** below.
...
BY default, Windows PowerShell doesn't return UTF-8, which can interfere with shell expansions generating foreign-language characters. The shell needs to be specifically configured to work with UTF-8 as in the following example:
```yml
 - trigger: ":pl"
    replace: "{{output}}"
    vars:
...
```
One equivalent solution for Python scripts in Windows is to include the following in the script:
```py
import sys
