...
### Inline scripts
Scripts can be included inline within an expansion by changing the `args:`, e.g.:
```py
          args:
            - python
            - -c
            - |
              ...
              for x in fruits:
                print(x)
```
This avoids the need to keep track of the script's location, but they're less easy to debug, and  Windows has an 8191 character limit on them.
...
```
It is also possible to embed anchors in trigger scripts with something like:
```yml
  - trigger: :test
    replace: '{{output}}'
    anchor: &script1 |
      print("Something")
```
with the `vars:` section being the same as before. The anchor will also be available for use in any other triggers that *follow*.
