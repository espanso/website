...
Like scripts, Windows PowerShell doesn't return UTF-8 characters, interfering with shell expansions generating foreign-language letters. The command needs to be specifically configured to work with UTF-8 as in the following example:
```yml
 - trigger: ":pl"
    replace: "{{output}}"
    vars:
