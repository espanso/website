...
If you only use a handful of snippets, placing everything in the `$CONFIG/match/base.yaml` file will work perfectly.
...
While you _could_ define them in the `$CONFIG/match/base.yaml` file, you prefer to keep
them separate from your other snippets.

For this reason, you create another file `$CONFIG/match/emails.yaml`, with the following content:

```yml title=$CONFIG/match/emails.yaml
matches:
  - trigger: ":sig"
    replace: "Best, John"
...
having all matches defined in the `$CONFIG/match/base.yaml` file.
...
For example, you could define a `$CONFIG/match/company.yaml` file that imports some matches from external locations:

```yaml title=$CONFIG/match/company.yaml
# Import other matches from external locations
imports:
  - "/path/to/other/matchsets.yaml"
  - "/path/to/shared/google/drive/matches.yaml"
...
For example, you could define a `$CONFIG/match/_js_snippets.yaml` file that includes some code-snippets for JavaScript
and a `$CONFIG/match/_css_snippets.yaml` file that contains some CSS snippets.
Because both files start with an underscore, they are not loaded automatically.

```yaml title=$CONFIG/match/_js_snippets.yaml
matches:
  - trigger: ":log"
    replace: "console.log($|$);"
```

```yaml title=$CONFIG/match/_css_snippets.yaml
matches:
  - trigger: ":pad"
    replace: "padding: 10px"
```

Then, you could define a `$CONFIG/match/_code_snippets.yaml` file that imports both of them:

```yaml title=$CONFIG/match/_code_snippets.yaml
imports:
  - "./_js_snippets.yaml"
  - "./_css_snippets.yaml"
```
