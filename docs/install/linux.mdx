...
| Fedora | **[RPM package](#rpm-x11) (recommended)**, [AppImage](#appimage-x11) (recommended), [Manual compilation](#x11-compile) | **[RPM package](#rpm-wayland) (recommended)**, [Manual compilation](#wayland-compile) |
...
### Installing with RPM package (Fedora) {#rpm-x11}

Espanso is available as a RPM on the [COPR website](https://copr.fedorainfracloud.org/coprs/eclipseo/espanso/) for Fedora.

Start by enable the COPR repository by running the following command inside a terminal:
...
* Install the required linux packages:
  ...
  * On **Fedora** run `sudo dnf install "pkgconfig(xcb)" "pkgconfig(x11)" "pkgconfig(xkbcommon)" "pkgconfig(xtst)" wxBase-devel  wxGTK-devel`
...
### Installing with RPM package (Fedora) {#rpm-wayland}
...
* Install the required linux packages:
  ...
  * On **Fedora** run `sudo dnf install @development-tools gcc-c++ wl-clipboard libxkbcommon-devel dbus-devel wxBase-devel wxGTK-devel`
* Espanso heavily relies on [cargo make](https://github.com/sagiegurari/cargo-make) for the various packaging
steps. You can install it by running:
