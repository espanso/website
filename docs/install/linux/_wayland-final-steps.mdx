...
If you can install `kdotool`, by whatever method is recommended for your distro, you may be able to use Espanso app-specific configurations in Wayland KDE Plasma.
...
keyboard_layout:
  layout: "it"
```
 The [`keyboard_layout:`](../../configuration/options.md/#options-reference) object accepts `rules:`, `model:`, `layout:`, `variant:` and `options:` fields. 

 Your current system settings may be displayed with one of the following commands: `setxkbmap -query`, `localectl status`, `swaymsg -t get_inputs`, `gsettings get org.gnome.desktop.input-sources sources` or `echo $XKB_DEFAULT_LAYOUT`, depending on your desktop environment.
